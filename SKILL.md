...
4. Normalize common airport, city, country/region, colloquial airport-name, airline, and alliance phrasing before choosing an endpoint; verify short carrier/alliance aliases locally and ask a targeted clarifying question when the normalized query maps to multiple plausible entities.
...
- Airport/city: prefer exact IATA/ICAO codes when supplied, but check known metro/city aliases (`NYC`, `LON`, `TYO`, etc.), country/region phrases (`Hawaii`, `Alaska`, `Southeast Asia`, etc.), and colloquial airport names (`Kennedy`, `Dulles`, `Heathrow`, `Changi`, etc.) before treating short text as one airport; otherwise search the city/airport phrase and preserve ambiguity instead of guessing.
